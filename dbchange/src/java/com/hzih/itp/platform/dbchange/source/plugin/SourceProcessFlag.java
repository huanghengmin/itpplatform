 ...
 */
public class SourceProcessFlag extends DbInit implements ISourceProcess {
    final static Logger logger = LoggerFactory.getLogger(SourceProcessFlag.class);
    ...
    public void init(SourceOperation source, DatabaseInfo databaseInfo) {
        ...
        flagName = databaseInfo.getFlagName();
    }
    ...
    public void run() {
        ...
        while (isRun) {
            exportOldData();
            LogLayout.info(logger, appName, "完成一次标记同步");
            try {
    ...
    private void disposeData(Connection conn, TableInfo tableInfo, InputStream in, Row[] rows) throws Ex {
        if (rows.length > 0) {
            ...
            if (result.getStatus().isSuccess()) {
               ...
               updateRows(conn, tableInfo, rows);
//                auditProcess(rows[0].getTableName(), rows.length);
    ...
    private void updateRows(Connection conn, TableInfo tableInfo, Row[] rows) throws Ex {
        String tableName = tableInfo.getName();
        for (Row row : rows) {
            updateRecord(row, tableName, conn);
        }
    }
     ...
     * @throws Ex
     */
    private void updateRecord(Row row, String tableName, Connection conn) throws Ex {
        PreparedStatement prepStmt = null;
        try {
            // String sqlString = "delete from {0   
            String sqlString = m_I_dbOp.getSqlProperty(conn, Str_Sql_UpdateSpecifyFlag,
            ...
            if (logger.isDebugEnabled()) {
                LogLayout.debug(logger,appName,"isSpecifyFlag : " + sqlString);
